<template>
    <div v-bind="$attrs">
        ...
        <img
            ...
            :class="reviewClass"
            :src="reviewData"
            style="border: solid 2px #e6e6e6;overflow: hidden;"
            :style="{
                borderRadius: arc + 'px',
                height: arc + 'px',
                width: arc + 'px'
    ...
    methods: {
        ...
        moveCvs(e) {
            if (this.isPoint) {
                ...
                let xv = offsetX - this.oldOffset.x
                let yv = offsetY - this.oldOffset.y

                if (
                    this.centreOfCircleX + radius > offsetX &&
                    this.centreOfCircleX - radius < offsetX &&
                    this.centreOfCircleY + radius > offsetY &&
                    this.centreOfCircleY - radius < offsetY
                ) {
                    let newX = this.centreOfCircleX + xv
                    let newY = this.centreOfCircleY + yv
                    ...
                    this.centreOfImgX = this.centreOfImgX + xv
                    this.centreOfImgY = this.centreOfImgY + yv
                }
        ...
        dwArc() {
            ...
            this.ctx.arc(
                this.centreOfCircleX,
                this.centreOfCircleY,
                this.lineWidth / 2 + radius,
                0,
                360,
                false
            )
        ...
        toDataURL() {
            ...
            let imgData = this.ctx.getImageData(
                this.centreOfCircleX - radius,
                this.centreOfCircleY - radius,
                radius * 2,
                radius * 2
            )
            ...
            this.$emit("cutDown", this.reviewData)
        }
    },
    mounted() {
        this.init()
